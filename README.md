...
- **Procesamiento Asíncrono (Anti-Congelamiento):** La codificación de video se delega a un hilo en segundo plano (`threading`) operando bajo el motor gráfico `Agg` de Matplotlib. Esto protege el hilo principal de Tkinter, asegurando que la interfaz no se congele ("No Responde") durante renderizados largos.
